...
// The Club sign-in link (v0.1.19): our own magic-link email, sent through
// Resend like everything else. Supabase only verifies the token; the email,
// the link, and the landing page are ours. Degrades gracefully like every
// other send path.
...
<div style="margin:0;padding:32px 16px;background:#F5F2EA;font-family:Georgia,'Times New Roman',serif;">
  <div style="max-width:560px;margin:0 auto;background:#FDFCF8;border:1px solid #E2DDCE;border-radius:14px;padding:40px 40px 32px;">
    <img src="${appUrl}/vitalis-mark.png" alt="Vitalis HealthCare" width="72" style="display:block;margin:0 auto 20px;" />
    <p style="text-align:center;font-size:13px;letter-spacing:0.18em;text-transform:uppercase;color:#5E9420;font-weight:bold;margin:0 0 6px;">Vitalis Care Club</p>
    <h1 style="text-align:center;font-size:26px;font-weight:600;color:#1D2A22;margin:0 0 24px;">Your sign-in link</h1>
    <p style="font-size:15px;line-height:1.65;color:#1D2A22;margin:0 0 16px;">Hello ${name},</p>
    <p style="font-size:15px;line-height:1.65;color:#1D2A22;margin:0 0 28px;">Use the button below to sign in to the Care Club portal. The link works once and expires after about an hour.</p>
    <div style="text-align:center;margin:0 0 28px;">
      <a href="${confirmUrl}" style="display:inline-block;background:#5E9420;color:#FFFFFF;font-family:Arial,sans-serif;font-size:15px;font-weight:bold;text-decoration:none;padding:14px 34px;border-radius:8px;">Sign in to the portal</a>
    </div>
    <p style="font-size:13px;line-height:1.6;color:#64705F;margin:0 0 6px;">If the button does not open, copy this link into your browser:</p>
    <p style="font-size:12px;line-height:1.6;color:#64705F;word-break:break-all;margin:0 0 24px;">${confirmUrl}</p>
    <p style="font-size:13px;line-height:1.6;color:#64705F;margin:0 0 24px;">If you did not request this link, you can safely ignore this email.</p>
    <p style="font-size:13px;line-height:1.6;color:#64705F;border-top:1px solid #EBE7DA;padding-top:18px;margin:0;">Vitalis Healthcare, LLC &middot; 8757 Georgia Avenue, Suite 440, Silver Spring, MD 20910 &middot; 240.716.6874</p>
  </div>
</div>`
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: FROM,
        to: [to],
        subject: 'Your Care Club sign-in link',
        html,
      }),
    })
    if (!res.ok) {
