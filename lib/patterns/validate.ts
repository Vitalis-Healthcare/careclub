...
//
// The scheduling grid (Option B ruling, updated 4 Aug 2026): visits are 2
// billed hours in 3-hour blocks. Block starts 8:00 / 11:00 / 14:00 give a
// caregiver three visits inside the 8:00-16:00 workday; each gap is 30 min
// finish-up + 20 min travel + 10 min slack. Capacity stays 120 hrs/month \u2014
...
export const BLOCK_STARTS = ['08:00', '11:00', '14:00'] as const
...
export function parsePatternSet(body: unknown): { patterns: PatternInput[] | null; error: string | null } {
  ...
  for (const item of raw) {
    ...
    if (!(BLOCK_STARTS as readonly string[]).includes(start)) {
      return { patterns: null, error: 'Visit times must be one of the standard blocks: 8:00 AM, 11:00 AM, or 2:00 PM.' }
    }
    const key = `${day}-${start}`
